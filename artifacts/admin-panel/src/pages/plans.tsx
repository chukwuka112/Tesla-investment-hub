  ...
  return (
    <div className="relative rounded-2xl overflow-hidden border border-white/10 bg-gradient-to-br from-zinc-900 via-zinc-800 to-zinc-900 shadow-xl">
      ...
      <div className="relative h-44 w-full overflow-hidden">
        ...
        {values.image_url ? (
          <img src={values.image_url} alt={values.name ?? "Plan"} className="h-full w-full object-cover" />
        ) : (
...
function PlanFormDialog({ open, onClose, editing, nextOrder, token, onSaved }: {
  open: boolean; onClose: () => void; editing: Plan | null; nextOrder: number; token: string | null; onSaved: () => void;
}) {
  ...
  const form = useForm<PlanFormValues>({
    ...
    defaultValues: editing ? {
      ...
      name: "", model_name: "", image_url: "",
      min_amount: 100, max_amount: 1000, no_max_limit: false,
      roi_percentage: 20, duration_days: 7, description: "",
      status: "active", display_order: nextOrder,
    },
  });
  ...
  const onSubmit = async (data: PlanFormValues) => {
    const payload = {
      name: data.name, model_name: data.model_name || null, image_url: data.image_url,
      min_amount: data.min_amount, max_amount: data.no_max_limit ? 9999999 : data.max_amount,
      roi_percentage: data.roi_percentage, duration_days: data.duration_days,
      description: data.description || null, status: data.status, display_order: data.display_order,
    };
    if (editing) {
      updatePlan.mutate({ id: editing.id, data: payload as any }, {
        onSuccess: () => { toast({ title: "Plan Updated" }); onSaved(); onClose(); },
        onError: (err: any) => toast({ title: "Update Failed", description: err?.data?.error || "Could not update plan", variant: "destructive" }),
      });
    } else {
      createPlan.mutate({ data: payload as any }, {
        onSuccess: () => { toast({ title: "Plan Created" }); onSaved(); onClose(); },
        onError: (err: any) => toast({ title: "Create Failed", description: err?.data?.error || "Could not create plan", variant: "destructive" }),
      });
    }
  };
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                <FormField control={form.control} name="image_url" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><ImageInput value={field.value} onChange={field.onChange} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                  ...
                  <FormField control={form.control} name="max_amount" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <div className="relative">
                          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground">$</span>
                          <Input type="number" min="1" placeholder="1000" disabled={noMaxLimit} className="bg-card/50 border-border/50 h-10 pl-6 disabled:opacity-40" {...field} />
                        </div>
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                ...
                <FormField control={form.control} name="description" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea placeholder="Describe this investment plan to users..." rows={3} className="bg-card/50 border-border/50 resize-none" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                <div className="grid grid-cols-2 gap-4">
                  <FormField control={form.control} name="status" render={({ field }) => (
                    <FormItem>
                      ...
                      <Select onValueChange={field.onChange} value={field.value}>
                        <FormControl>
                          <SelectTrigger className="bg-card/50 border-border/50 h-10"><SelectValue /></SelectTrigger>
                        </FormControl>
                        <SelectContent>
                  ...
                  <FormField control={form.control} name="display_order" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input type="number" min="0" className="bg-card/50 border-border/50 h-10" {...field} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                </div>
...
function AdminPlanCard({ plan, isFirst, isLast, onEdit, onToggle, onDelete, onMoveUp, onMoveDown, loading }: {
  plan: Plan; isFirst: boolean; isLast: boolean; onEdit: () => void; onToggle: () => void; onDelete: () => void;
  onMoveUp: () => void; onMoveDown: () => void; loading: boolean;
}) {
  const noMax = plan.max_amount >= 999999;

  return (
    <div className={`relative flex gap-4 rounded-xl border bg-card p-4 transition-all ${plan.status === "active" ? "border-border/60" : "border-border/30 opacity-60"}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2 mb-1">
          <div>
            ...
            {plan.model_name && <p className="text-xs text-muted-foreground mt-0.5">{plan.model_name}</p>}
          </div>
          <span className={`flex-shrink-0 text-xs px-2 py-0.5 rounded-full font-medium ${plan.status === "active" ? "bg-green-500/20 text-green-400" : "bg-zinc-500/20 text-zinc-400"}`}>
        ...
        {plan.description && <p className="text-xs text-muted-foreground mt-2 line-clamp-1">{plan.description}</p>}
      </div>
      <div className="flex flex-col items-end justify-between flex-shrink-0 gap-2">
        <div className="flex items-center gap-1">
          <button onClick={onMoveUp} disabled={isFirst || loading}
          ...
          <button onClick={onMoveDown} disabled={isLast || loading}
            className="h-7 w-7 rounded flex items-center justify-center text-muted-foreground hover:text-white hover:bg-card border border-transparent hover:border-border/50 transition-all disabled:opacity-30 disabled:cursor-not-allowed">
        ...
        <div className="flex items-center gap-1">
          <button onClick={onToggle} disabled={loading}
            className={`h-7 px-2 text-xs rounded border transition-all disabled:opacity-50 ${plan.status === "active" ? "border-green-500/40 text-green-400 hover:bg-green-500/10" : "border-zinc-500/40 text-zinc-400 hover:bg-zinc-500/10"}`}>
...
export default function AdminPlans() {
  ...
  const invalidate = () => queryClient.invalidateQueries({ queryKey: getAdminGetPlansQueryKey() });

  const handleToggleStatus = async (plan: Plan) => {
    setActionLoading(plan.id);
    const newStatus = plan.status === "active" ? "inactive" : "active";
    updatePlan.mutate({ id: plan.id, data: { status: newStatus } as any }, {
      onSuccess: () => {
        toast({ title: newStatus === "active" ? "Plan Enabled" : "Plan Disabled" });
        invalidate();
      },
  ...
  const handleSeed = async () => {
    ...
    try {
      ...
      if (!res.ok) { toast({ title: "Seed Failed", description: json.error, variant: "destructive" }); return; }
  ...
  return (
    <AdminLayout title="Investment Plans">
      <div className="mb-6 flex items-center justify-between gap-4 flex-wrap">
        <p className="text-sm text-muted-foreground">{plans.length} plans total · {activePlans} active</p>
        <div className="flex items-center gap-2">
          {plans.length === 0 && (
          ...
          <Button onClick={() => { setEditingPlan(null); setDialogOpen(true); }} className="bg-primary hover:bg-primary/90">
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[1, 2, 3].map((i) => <div key={i} className="h-32 rounded-xl border border-border/40 bg-card animate-pulse" />)}
        </div>
      ) : sorted.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center border border-dashed border-border/50 rounded-xl">
          ...
          <p className="text-sm text-muted-foreground mb-6 max-w-xs">Create investment plans that users can browse and invest in.</p>
