              ...
              <TableRow key={w.id} className="border-border/50">
                ...
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-2">
                    {w.status === "pending" && (
                      <>
                        <Button variant="default" size="sm" onClick={() => apiAction(w.id, "approve")} disabled={loadingId === w.id}>Approve</Button>
                        <Button variant="destructive" size="sm" onClick={() => apiAction(w.id, "reject")} disabled={loadingId === w.id}>Reject</Button>
                      </>
                    )}
                    {w.status === "approved" && (
                      <Button variant="outline" size="sm" className="border-green-500/50 text-green-400 hover:bg-green-500/10 hover:text-green-300" onClick={() => apiAction(w.id, "complete")} disabled={loadingId === w.id}>
                        Mark Completed
                      </Button>
                    )}
