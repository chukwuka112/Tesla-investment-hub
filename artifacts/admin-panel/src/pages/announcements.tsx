              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                ...
                <FormField control={form.control} name="content" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea placeholder="Write the announcement message..." rows={5} className="bg-card/50 border-border/50 resize-none" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
  ...
  return (
    <div className={`rounded-xl border bg-card p-5 transition-all ${ann.is_active ? "border-border/60" : "border-border/30 opacity-60"}`}>
      ...
      <div className="flex items-center gap-2 ml-9 flex-wrap">
        <button onClick={() => patch({ is_active: !ann.is_active })} disabled={loading}
        ...
        <button onClick={() => patch({ is_pinned: !ann.is_pinned })} disabled={loading}
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center border border-dashed border-border/50 rounded-xl">
          ...
          <p className="text-sm text-muted-foreground mb-6 max-w-xs">
            Broadcast messages to all users — promotions, maintenance notices, updates.
          </p>
          {filter === "all" && (
            <Button onClick={() => { setEditingAnn(null); setDialogOpen(true); }} className="bg-primary hover:bg-primary/90">
        ...
        <div className="space-y-3">
          {filtered.map((ann) => (
            <AnnouncementCard key={ann.id} ann={ann} token={token}
